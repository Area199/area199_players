...
def calculate_dynamic_score(test_col_name, raw_value, birth_year, df_all_tests, lower_is_better=False):
    ...
    try:
        ...
        # Conversione in percentile tramite funzione Erf
...
def fetch_player_payload(name_query):
    """Fetch centralizzato di tutti i dati necessari"""
    try:
        sh = get_db()
        # 1. Anagrafica Atleta
        ...
        # 2. Archivio Test (Caricamento raw per calcoli dinamici)
...
with header_col2:
    if st.session_state.auth_payload is not None:
        ...
        if st.button("🔴 LOGOUT", key="logout_btn", kind="secondary"):
            st.session_state.auth_payload = None
            st.rerun()
...
if st.session_state.auth_payload is None:
    # --- VIEW: LOGIN ---
    st.markdown("<br><h3 style='text-align:center;'>PERFORMANCE HUB LOGIN</h3>", unsafe_allow_html=True)
            ...
            if found_cred:
                with st.spinner("Sincronizzazione dati in corso..."):
                    ...
                    if payload:
                        ...
                        st.error("Atleta riconosciuto ma non trovato nel database PLAYERS.")
            else:
    ...
    st.write(f"Accesso: **{info['Nome']} {info['Cognome']}**")
    st.divider()
    ...
    if not my_tests.empty:
        ...
        # Calcolo Score Z-Score (Formula allineata all'Hub Coach)
        ...
        # AI FEEDBACK DOTT. PETRUZZI
        try:
            client = openai.OpenAI(api_key=st.secrets.get("openai_key") or st.secrets.get("openai_api_key"))
            resp = client.chat.completions.create(
                model="gpt-4o", 
                messages=[{"role": "system", "content": f"Sei il Dott. Petruzzi, AREA199. Analizza Gerardo Petruzzi: VEL:{scores[0]}, AGI:{scores[1]}, FIS:{scores[2]}, RES:{scores[3]}, TEC:{scores[4]}. Ruolo: {info['Ruolo']}. Sii tecnico e motivante. Max 50 parole."}]
            )
            st.markdown(f"""
        ...
        # RADAR CHART DUAL (TARGET VERDE VS ATTUALE ROSSO) - NO ZOOM
        ...
        fig.update_layout(
            ...
            dragmode=False # Disabilita trascinamento
        )
